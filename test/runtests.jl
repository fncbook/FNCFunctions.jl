...
@testset "Chapter 2" begin
    ...
    L, U = FNC.lufact(A)
    @test isapprox(L * U, A, rtol = 100eps())
    @test all(U .== triu(U))
    @test all(L .== tril(L))
    b = [1, 10, 0, -1] / 5;
    @test isapprox(L\b, FNC.forwardsub(L,b), rtol=100eps())
    @test isapprox(U\b, FNC.backsub(U,b), rtol=100eps())
    L, U, p = FNC.plufact(A)
    @test isapprox(L*U, A[p,:], rtol=100eps())
    @test all(U .== triu(U))
    @test all(L .== tril(L))
end
